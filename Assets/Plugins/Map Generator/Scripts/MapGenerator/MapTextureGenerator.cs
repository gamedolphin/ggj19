...
public static class MapTextureGenerator
{
    ...
    private static Texture2D CreateTextureFromRenderTexture(int textureSize, RenderTexture renderTexture)
    {
        ...
        File.WriteAllBytes(Application.dataPath + "/../Terrain_Texture.png", bytes);
        Debug.Log(string.Format("Texture Saved: {0:n0}ms", DateTime.Now.Subtract(time).TotalMilliseconds));
